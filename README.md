...
- `output/<language>/WeaponActionValues.xlsx`: one sheet per weapon plus `Ammo`. Rows without a verified action name stay at the bottom with a blank `ActionName` cell and orange fill.
...
`WeaponActionValues.xlsx` reads mappings only from `MHWS-in-json/ActionMap.json` (`_format = mhws_static_action_request_set_map_v1`). Generate this file locally with `motlist-to-json action-map` using inputs from the same game version: the executable dump, `il2cpp_dump.json`, motbank/motlist resources, PAK file list, and MHWS JSON. Set `MHWS_ACTION_MAP_PATH` to test a file elsewhere. CI reads the committed package and does not need the generator inputs.

Only player attack-value requestSets are exported. Nested values are flattened to leaf columns, and an RS is repeated when it maps to more than one displayed action name.

Each relation is keyed by `(scope, rcol, requestSetID, keyHash, sourceRequestSetOrdinal)`. Export stops if a key does not match the current RCOL data or if relations conflict. A relation without `actionNameGuid` receives no action name; its `ActionName` cell stays blank in the orange block at the bottom. Many-to-many mappings are preserved. Duplicate RS rows under the same displayed name are removed, and actions with the same localized name are merged.
