...
from src.shared.excel.cells import safe_cell as _cell, sheet_name as _sheet_name, text_width as _text_width
from src.shared.tables import as_list as _as_list
...
def export_bowgun_workbooks(output_dir: Path, natives_dir: Path, text_source: TextSource,
                            repository: SourceRepository | None = None) -> None:
    repository = repository or SourceRepository(natives_dir)
    text_db = text_source.build(ZH_HANS_LANGUAGE_ID)

    info("    Loading bowgun customize tables")
    customize_rows = _customize_rows(repository, text_db)
    ...
    sheets = {
        "Wp_HeavyBowgun": _load(repository, HEAVY_BOWGUN_PATH, text_db),
        "Wp_LightBowgun": _load(repository, LIGHT_BOWGUN_PATH, text_db),
    }
    sheets = prepare_equipment(
        sheets,
        lambda relative_path: _load(repository, relative_path, text_db),
    )
    sheets = localize(sheets, text_db.get)
...
def _customize_rows(repository: SourceRepository, text_db: TextDB) -> Table:
    patterns = _load(repository, CUSTOMIZE_DATA_PATH, text_db)
    items = {
        row.get("ItemID"): row
        for row in _load(repository, CUSTOMIZE_ITEM_PATH, text_db)
    }
...
def _load(repository: SourceRepository, relative_path: str, text_db: TextDB) -> Table:
    frame = localize(repository.referenced_table(relative_path), text_db.get)
