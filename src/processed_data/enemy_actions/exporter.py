...
def load_enemy_action_catalog(
    ...
    candidates_by_prefix = _enemy_candidates(natives_dir, text_db, repository)
...
def _enemy_candidates(
    ...
    rows = (localize(repository.referenced_table(SUPPORT_FILES["enemy"]), text_db.get)
            if repository else load_user3_table(enemy_path, text_db))
