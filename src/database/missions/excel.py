...
def write_mission_workbook(path: Path, data: MissionWorkbookData) -> Path:
    ...
    for row in data.rows:
        sheet.append([safe_cell(row.get(header.key)) for header in data.headers])
...
def _body_styles(palette: StylePalette, headers, group_starts: set[int]) -> dict:
    styles = {}
    column_styles = dict.fromkeys(
        (header.section, index in group_starts, header.key in WRAPPED_COLUMNS)
